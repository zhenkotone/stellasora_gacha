...
- 本项目是非官方的个人研究工具，与《星塔旅人》运营方、开发方不存在关联，也未获得官方认可或授权。
- 工具设计为仅通过 Windows `ReadProcessMemory` 读取本机当前登录客户端中已加载的招募历史；不注入 DLL、不修改游戏文件或内存、不发送游戏协议请求。实现说明不构成对账号安全、封号风险或长期可用性的保证。
- 游戏客户端、反作弊策略、服务条款和本地法律法规都可能变化。使用者应自行阅读并遵守适用条款，评估风险，并仅对自己的账号和设备使用本工具。
- 因使用、无法使用、数据不完整、游戏更新、账号处置或第三方服务变更造成的任何直接或间接损失，项目维护者不承担责任。
- 请自行备份 `exports/stellasora_gacha_archive.json`，并勿公开分享其中可能关联个人游戏行为的导出数据。
